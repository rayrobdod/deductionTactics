...
class MoveRangeOfTest extends FunSpec {
	...
	describe ("moveRangeOf") {
		it ("A healthy current-turn Token has a full range") {
			val token = new Token(board.space((1,1)).get, tokenClass = myTokenClass).startOfTurn()
			val list = new ListOfTokens(Seq(Seq(token)))
			
			assertResult(11)(moveRangeOf(token, list).size)
		}
		it ("A healthy Token has a full speed") {
			val token = new Token(board.space((1,1)).get, tokenClass = myTokenClass)
			val list = new ListOfTokens(Seq(Seq(token)))
			
			assertResult(11)(moveRangeOf(token, list).size)
		}
		it ("A SnakeToxin'd Token has a speed of one") {
			val token = new Token(board.space((1,1)).get, tokenClass = myTokenClass, currentStatus = Statuses.Snake)
			val list = new ListOfTokens(Seq(Seq(token)))
			
			assertResult(5)(moveRangeOf(token, list).size)
		}
		it ("A Sleep'd Token has a speed of zero") {
			val token = new Token(board.space((1,1)).get, tokenClass = myTokenClass, currentStatus = Statuses.Sleep)
			val list = new ListOfTokens(Seq(Seq(token)))
			
			assertResult(1)(moveRangeOf(token, list).size)
		}
		it ("A Burn'd Token has a full speed") {
			val token = new Token(board.space((1,1)).get, tokenClass = myTokenClass, currentStatus = Statuses.Burn)
			val list = new ListOfTokens(Seq(Seq(token)))
			
			assertResult(11)(moveRangeOf(token, list).size)
		}
		it ("An unknown token guesses no speed") {
			val token = new Token(board.space((1,1)).get).startOfTurn()
			val list = new ListOfTokens(Seq(Seq(token)))
	...
	describe ("attackRangeOf") {
		...
		it ("A Blind'd Token has a full speed") {
			val token = new Token(board.space((1,1)).get, tokenClass = myTokenClass, currentStatus = Statuses.Blind)
			val list = new ListOfTokens(Seq(Seq(token)))
	...
	describe ("MoveToCostFunction") {
		it ("UniPassageSpaceClass occupied") {
			...
			assertResult(SpaceClass.impossiblePassage)(
					new MoveToCostFunction(token, list)(board.space((1,1)).get, board.space((1,1)).get)
			)
		}
		it ("UniPassageSpaceClass unoccupied") {
			...
			assertResult(SpaceClass.normalPassage)(
					new MoveToCostFunction(token, list)(board.space((2,2)).get, board.space((2,2)).get)
			)
		}
	}
