...
{
	val hilights:Map[BoardGameSpace, JLabel] = panel.fieldComp.spaces.map{(space:RectangularSpace) =>
		val label = new JLabel 
		label.setIcon(null)
		panel.centerpiece.add(label,0)
		(space, label)
	}.toMap
	...
	override def apply(e:Event) = {
		...
		hilights.foreach({(space:BoardGameSpace, label:JLabel) =>
			space match{
				...
				{
					val hilightedLabel = panel.fieldComp.spaceLabelMap(rs)
					label.setIcon(null)
					label.setLocation(hilightedLabel.getLocation)
					label.setSize(hilightedLabel.getSize)
				}
			}
		}.tupled)
